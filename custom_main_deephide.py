"""
This code is modified from: https://github.com/ChaoningZhang/Universal-Deep-Hiding/
"""
...
parser.add_argument('--batch_save_frequency', type=int, default=100, help='how often to save in a batch')
parser.add_argument('--experiment_dir', type=str, default='models/steg')
...
def main():
    ...
    if not opt.debug:
        try:
            ...
            if opt.test == '':
                ...
                save_dir = f"{opt.experiment_dir}/{opt.remark}_{opt.data}"
                opt.outckpts = save_dir+"/checkPoints"
                opt.trainpics = save_dir + "/trainPics"
                opt.validationpics = save_dir + "/validationPics"
                opt.outlogs = save_dir + "/trainingLogs"
                opt.outcodes = save_dir + "/codes"
    ...
    if opt.test == '':
        ...
        # load the trainsets
        if opt.data == "mnist":
            train_dataset_cover = torchvision.datasets.MNIST(root='./data', train=True,
                                                    download=True, transform=transforms_cover)
            val_dataset_cover = torchvision.datasets.MNIST(root='./data', train=False,
                                                   download=True, transform=transforms_cover)
            
            train_dataset_secret = torchvision.datasets.MNIST(root='./data', train=True,
                                                    download=True, transform=transforms_secret)
            val_dataset_secret = torchvision.datasets.MNIST(root='./data', train=False,
                                               download=True, transform=transforms_secret)
        elif opt.data == "cifar":
            train_dataset_cover = torchvision.datasets.CIFAR10(root='./data', train=True,
                                                    download=True, transform=transforms_cover)
            val_dataset_cover = torchvision.datasets.CIFAR10(root='./data', train=False,
                                                   download=True, transform=transforms_cover)
            
            train_dataset_secret = torchvision.datasets.CIFAR10(root='./data', train=True,
                                                    download=True, transform=transforms_secret)
            val_dataset_secret = torchvision.datasets.CIFAR10(root='./data', train=False,
                                               download=True, transform=transforms_secret)
            ...
            val_dataset_secret = ImageFolder(
                valdir, 
                transforms_secret)
        # train_dataset_cover, val_dataset_cover = torch.utils.data.random_split(
        # root="./data", train=True, [50000, 10000], download=True, generator=torch.Generator().manual_seed(1), transform=transforms_cover)
        ...
        if opt.data == "mnist":
            ...
            test_dataset_secret = torchvision.datasets.MNIST(root='./data', train=True,
                                        download=True, transform=transforms_secret)
            test_dataset_cover = torchvision.datasets.MNIST(root='./data', train=False,
                                       download=True, transform=transforms_cover)
            
        elif opt.data == "cifar":
        
            test_dataset_secret = torchvision.datasets.CIFAR10(root='./data', train=True,
                                        download=True, transform=transforms_secret)
            test_dataset_cover = torchvision.datasets.CIFAR10(root='./data', train=False,
                                       download=True, transform=transforms_cover)
    ...
    if opt.checkpoint != "":
        if opt.checkpoint_diff != "":
            ...
            checkpointD = torch.load(opt.checkpoint_diff, map_location=torch.device('cpu'))
            HnetD.load_state_dict(checkpointD['H_state_dict'])
            RnetD.load_state_dict(checkpointD['R_state_dict'])
        else:
            # print("there")
            checkpoint = torch.load(opt.checkpoint, map_location=torch.device('cpu'))
            # checkpoint_diff = torch.load(opt.checkpoint_diff, map_location=torch.device('cpu'))
            H_state = {}
            R_state = {}
    ...
    if opt.test == '':
        # if not opt.debug:
        #     writer = SummaryWriter(log_dir='runs/' + opt.experiment_dir) 
...
def train(train_loader, epoch, Hnet, Rnet, criterion):
    ...
    for i, ((secret_img, secret_target), (cover_img, cover_target)) in enumerate(train_loader, 0):
        ...
        # print(errH)
        # print(errR.data[0])
        # print(errR.data[0].shape)
        Hlosses.update(errH.data.item(), opt.bs_secret*opt.num_cover*opt.num_training)  # H loss
        Rlosses.update(errR.data.item(), opt.bs_secret*opt.num_secret*opt.num_training)  # R loss
        Hdiff.update(diffH.data.item(), opt.bs_secret*opt.num_cover*opt.num_training)
        Rdiff.update(diffR.data.item(), opt.bs_secret*opt.num_secret*opt.num_training)
        ...
        if i % opt.batch_save_frequency == 0:
            save_checkpoint({
                'epoch': epoch + 1,
                'batch': i+1,
                'H_state_dict': Hnet.state_dict(),
                'R_state_dict': Rnet.state_dict(),
                'optimizer' : optimizer.state_dict(),
            }, 0, epoch, '%s/epoch_%d-batch_%d_Hloss_%.4f_Rloss=%.4f_Hdiff_Hdiff%.4f_Rdiff%.4f' % (opt.outckpts, epoch, i, Hlosses.val, Rlosses.val, Hdiff.val, Rdiff.val ))
...
def save_result_pic_analysis(bs_secret_times_num_training, cover, container, secret, rev_secret, epoch, i, save_path=None, postname=''):
    ...
    for i_cover in range(4):
        ...
        if i_cover == 0:
            ...
            print(showCover.shape, cover_i.shape, container_i.shape, cover_gap_i.shape)
